exports.run = async (client, msg, args) => {
   ...
   color: Math.floor(Math.random() * (0xFFFFFF + 1)),
   description: (`:no_entry_sign: Hoop! Birşeyler ters gitti kennethi etiketlemişsin etiketlemişsin ona küfür etmem lan!`)
  }})
    if(member.id === "203116573211230208")return msg.channel.send({embed: {
   color: Math.floor(Math.random() * (0xFFFFFF + 1)),
   description: (`:no_entry_sign: Hoop! Orda durucan Yapımcım kenneth'ya sövemezsin .`)
  }})
